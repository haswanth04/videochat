...
async function handleConnect() {
    ...
    try {
        ...
        // Then try to get video (but don't fail if we can't)
        try {
        ...
        // Continue with setup even if only audio is available
        getHtmlElementsById();
        setButtonsToolTip();
        ...
        // Join the channel after media setup
        await joinToChannel();
    } catch (err) {
        console.error('Error in setup:', err);
        msgPopup('error', 'Failed to access media devices: ' + err.message, 'top-end', 6000);
    }
}
